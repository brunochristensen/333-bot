...
public class ReminderCommand extends ListenerAdapter {
    ...
    public void onReady(@NotNull ReadyEvent event) {
        try {
            handler = new ReminderHandler(event.getJDA());
        } catch (SchedulerException e) {
            Objects.requireNonNull(event.getJDA().getTextChannelById(Env.get("DEBUG_CHANNEL_ID")))
                    .sendMessageEmbeds(EmbedResponse.error("ReminderHandler failed to fetch scheduler."))
                    .queue();
    ...
    public void onSlashCommandInteraction(@NotNull SlashCommandInteractionEvent event) {
        if (event.getName().equals("account")) {
            if (Objects.requireNonNull(event.getMember())
                ...
                event.replyEmbeds(controlMenuEmbed)
                        .addActionRow(Button.success("remAdd", "Add New Reminder").asDisabled())
                        .addActionRow(Button.danger("remDel", "Delete Reminder").asDisabled())
                        .addActionRow(Button.secondary("remView", "View  Reminders").asDisabled())
                        .setEphemeral(true)
                        .queue();
            }
        }
    ...
    public void onButtonInteraction(@NotNull ButtonInteractionEvent event) {
        if (event.getComponentId().equals("remAdd")) {
            handler.addJob();
            handler.addTrigger();
        } else if (event.getComponentId().equals("remDel")) {
            handler.delTrigger();
            handler.delJob();
        } else if (event.getComponentId().equals("remView")) {
