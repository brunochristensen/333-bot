...
public class AccountabilityMenuListener extends MenuListener {
    ...
    private final Modal additionModal = Modal.create("newAccModal", "New Accountability Schedule")
            .addComponents(ActionRow.of(
                            TextInput.create("triggerNameAddTextInput", "TriggerName", TextInputStyle.SHORT)
                                    .setPlaceholder("Unique name for Trigger").setRequiredRange(1, 50).build()),
                    ActionRow.of(TextInput.create("cronSchTextInput", "CronSch", TextInputStyle.SHORT)
                            .setPlaceholder("CronJob formatted schedule").setRequiredRange(1, 50).build()),
                    ActionRow.of(TextInput.create("uniformTextInput", "Uniform", TextInputStyle.SHORT)
                            .setPlaceholder("Uniform of the Day").setRequiredRange(1, 50).build()),
                    ActionRow.of(TextInput.create("timeTextInput", "Time", TextInputStyle.SHORT)
                            .setPlaceholder("Time (e.g. 0550)").setRequiredRange(4, 4).build()),
                    ActionRow.of(TextInput.create("locationTextInput", "Location", TextInputStyle.SHORT)
                            .setPlaceholder("Where accountability is taking place").setRequiredRange(1, 50)
                            .build()))
            .build();
    private final Modal deletionModal = Modal.create("delAccModal", "Delete Accountability Schedule")
            .addComponents(ActionRow.of(
                    TextInput.create("triggerNameDelTextInput", "TriggerName", TextInputStyle.SHORT)
                            .setPlaceholder("Unique name of Trigger").setRequiredRange(1, 100).build())).build();
    ...
    public void onButtonInteraction(@NotNull ButtonInteractionEvent event) {
        switch (AccountabilityComponentID.valueOf(event.getComponentId())) {
            case ADDBUTTON:
                ...
                break;
            case DELBUTTON:
                event.replyModal(deletionModal).queue();
                break;
            case VIEWBUTTON:
                try {
                    Map<String, Map<String, String>> data = handler.getTriggerData();
                    event.replyEmbeds(getTriggerDataEmbed(data)).setEphemeral(true).queue();
                ...
                try {
                    ...
                    if (handler.skipNextTrigger()) {
                        reply = event.replyEmbeds(
                                EmbedResponse.success("The next Trigger will skip it's fire time.").build());
                    } else {
                        reply = event.replyEmbeds(
                                EmbedResponse.failure("There are no Triggers to skip.").build());
                    }
            ...
            case GETBUTTON:
                ...
                event.replyEmbeds(getAccountabilityResponseEmbed(data)).setEphemeral(true).queue();
                break;
            default:
                break;
        }
    ...
    private MessageEmbed getAccountabilityResponseEmbed(@NotNull Map<String, String> data) {
        EmbedBuilder tmp = EmbedResponse.success(
                "The following individuals have reported for accountability:");
    ...
    private MessageEmbed getTriggerDataEmbed(@NotNull Map<String, Map<String, String>> data) {
        EmbedBuilder tmp = EmbedResponse.success(
                "Here are the currently scheduled accountability Jobs:");
        if (data.isEmpty()) {
